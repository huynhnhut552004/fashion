...
const APIproduct= "https://fashion-bsqk.onrender.com/Product/Womenproduct";
...
document.getElementById("Category").addEventListener("change", async ()=>{
  ...
  const res= await fetch(`${APICategory}/${category}/gender/Women`);
  const data= await res.json();
  const product= document.getElementById("Area-Product");
  product.innerHTML="";
  data.forEach(p=>{
...
async function search() {
  const query= input.value.trim();
    ...
    timer= setTimeout(()=>{
        fetch(`https://fashion-bsqk.onrender.com/Product/searchWomen?search=${encodeURIComponent(query)}`)
        .then(res => {
...
async function quantity() {
  ...
  if(!token){
    ...
    const cart= await fetch("hhttps://fashion-bsqk.onrender.com/Cart", {
